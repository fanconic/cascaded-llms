...
from src.utils import plot_accuracy_vs_cost, plot_decision_distribution
...
class Experiment:
    ...
    def _update_collectors(self, collectors: Dict, decision: Dict, label: str) -> None:
        ...
        pred_letter = extract_predictions(decision['response'].strip()) if decision['response'] else None
    ...
    def _create_dataframe_dict(self, collectors: Dict) -> Dict:
        ...
        return {
            ...
            "base_prediction": [extract_predictions(pred) for pred in collectors['all_small_predictions']],
            "large_response": collectors['all_large_predictions'],
            "large_prediction": [extract_predictions(pred) for pred in collectors['all_large_predictions']],
    ...
    def _calculate_and_plot_metrics(
        self, data: pd.DataFrame, dataset_length: int
    ) -> None:
